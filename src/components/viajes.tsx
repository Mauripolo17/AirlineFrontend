...
import { Vuelo2, vueloService } from "../api/vuelosService";
import ciudades from "../assets/images/ciudades.tsx";

export default function Viajes() {
  const [vuelos, setVuelos] = useState<Vuelo2[]>([]);
  ...
  const loadVuelos = async () => {
    try {
      ...
      const vuelosConImagen = getVuelos.map((vuelo) => ({
        ...vuelo,
        imagen: obtenerAtributoAleatorio(),
      }));
      setVuelos(vuelosConImagen);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const itemTemplate = (data: Vuelo2) => {
    return (
